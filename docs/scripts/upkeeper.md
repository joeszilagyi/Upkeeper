...
Important:
  ...
  - Run backlog batches with `orchestration/backlog_loop.sh`, or call
    ...
    their timestamp and bold marker text; `PAGE` puts the timestamp in white on
    a red background and highlights the `ERROR` text inside `[ERROR]` in the
    same red/blink style as the `PAGE` marker. Loop logs keep the same block
