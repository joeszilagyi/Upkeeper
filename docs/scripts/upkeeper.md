...
Important:
  ...
  - Before normal GitHub issue selection, the backlog launcher scans recent
    ...
    After anomaly custody and reconciliation, backlog also maps open
    current-root obligations to existing issue links by stable obligation
    signature, then writes or refreshes one deterministic issue-ready report for
    every current-root obligation before selecting work, so system-level
    failures have durable issue text even when backend Codex never enters
    bug-report-only mode. The launcher summary reports scanned, linked,
    issue-ready, GitHub-created, deferred, and still-local-only counts. These
    local reports default to
