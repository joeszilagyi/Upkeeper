...
show_help() {
  ...
  cat <<EOF
Usage: $SCRIPT_NAME [--help] [--version] [--status] [--doctor] [--last-run] [--open-failures] [--quota-status] [--json-status] [--config-file=PATH] [--no-config] [--prompt-file FILE] [--prompt TEXT] [--review-module=$review_module_pipe] [--review-modules=$review_module_csv] $review_module_shorthand_usage [--model-override=5.5_xhigh|5.3-codex-spark_xhigh] [--target-file=PATH] [--target-root=PATH] [--target-depth=N] [--selection-source=manifest|enumerate] [--selection-order=oldest|newest|random] [--select-untracked[=0|1]] [--tracked-only] [--refresh-manifest] [--manifest-file=PATH] [--allow-unsafe-manifest-path] [--include-glob=PATTERN] [--include-globs=a,b] [--exclude-glob=PATTERN] [--exclude-globs=a,b] [--selection-review-modules=$review_module_csv] [--ignore-failure-queue] [--backup-queue] [--prompt-pass=all] [--max-cover] [--bug-report-only] [--audit-only] [--fix-next-issue] [--fix-issue=NUMBER] [--issue-workflow-stage=comment|review|apply]
...
Operator status commands:
  --status         Human-readable local wrapper health summary.
  --doctor         Human-readable dependency, active-lock, and finding summary.
  --last-run       Last logged cycle/result summary from the local log tail.
  --open-failures  Count open local tool-failure and automation-obligation evidence.
  --quota-status   Current local quota snapshot summary for the selected model.
  --json-status    Machine-readable local status document, schema upkeeper.status.v1.

Status commands are deterministic local reads. They do not acquire the active
run lock, launch backend Codex, call GitHub, or mutate runtime evidence.
...
Loop stop semantics:
  ...
  - root log rotation archives live logs older than ${CODEX_LOG_ROTATE_AFTER_HOURS} hours
    ...
    Custom CODEX_LOG_FILE paths are still honored as live log sinks, but
    rotation and sibling archive pruning stay blocked unless explicitly enabled
    with CODEX_LOG_FILE_ALLOW_UNSAFE=1 and a trusted Upkeeper rotation marker.
  - open breadcrumb custody records under UPKEEPER_BREADCRUMB_STATE_DIR with
    severity critical or high redirect normal target rotation to the configured
    Upkeeper gate target before backend work starts. Explicit target pins and
    issue-fix pins remain visible as pinned work and are not silently replaced.
...
Important:
  ...
  - Before normal GitHub issue selection, the backlog launcher scans recent
    ...
    instead of opening a new obligation for each cycle id or run hash. When one
    source cycle emits multiple hard control-plane signals, anomaly custody
    ...
    target, issue, and stable fingerprint are condensed to one active owner.
    System-level wrapper failures such as empty transcripts and child exits are
    instead grouped by failure class, reason, target, and repair target so
    per-cycle output fingerprints cannot create a public issue cascade.
    Records already linked to the same specific issue title and issue number
    also reconcile as one local owner even when their evidence fingerprints
    differ. Duplicates are moved to resolved evidence with duplicate_of
    metadata.
  ...
  - Startup scans the recent live log for prior cycles that started but never
    ...
    previous_run.anomaly_summary, preserves previous_run.anomaly_detail records
    in local evidence, and injects those findings into the prompt for the next
    healthy run. Evidence already covered by a current-root prior-run/startup
    anomaly obligation logs previous_run.known_anomaly_residue at INFO and does
    not reopen the startup gate.
