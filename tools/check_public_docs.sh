...
version/doc sync, required P26/P27 policy wiring, broken repo-local Markdown
links, and obvious placeholder/legalese public text.
EOF
    exit 0
    ;;
  *)
...
grep -Fq "$wrapper_version changes" change_notes.md || fail "change_notes.md has no entry for $wrapper_version"
...
git diff --check -- README.md AGENTS.md change_notes.md docs lib/upkeeper/README.md prompts templates tools/check_public_docs.sh
