...
grep -Fq "P27 - After-Action Review" prompts/p27-educational-debrief-review.md || fail "P27 prompt title missing"
...
grep -Fq "tools/validate_upkeeper.sh --smoke" tools/docs_only_fast_path.sh || fail "docs-only helper does not run smoke validation"
...
grep -Fq "tools/run_validation_phases.sh --phases shell_syntax,unit_tests,public_docs,diff_whitespace" .github/workflows/ci.yml || fail "CI workflow does not run parallel local validation gates"
